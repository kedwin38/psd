...
export class TokenService {
  ...
  async issueRefreshToken(userId: string, opts: { familyId?: string; ip?: string; userAgent?: string }): Promise<IssuedRefreshToken> {
  ...
  async rotateRefreshToken(rawToken: string, opts: { ip?: string; userAgent?: string }): Promise<{ userId: string; issued: IssuedRefreshToken }> {
    ...
    await this.prisma.refreshToken.update({
      ...
      data: { revokedAt: new Date(), replacedByHash: this.hashOpaqueToken(issued.raw) },
    });
